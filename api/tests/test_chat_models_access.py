...
def test_models_endpoint_overlays_user_and_sponsored_capacity(
    ...
    body = response.json()
    luna = next(model for model in body["models"] if model["id"] == LUNA_ID)
...
def test_models_endpoint_uses_one_cached_base_for_different_users(monkeypatch) -> None:
    ...
    first_luna = next(
        model for model in first.json()["models"] if model["id"] == LUNA_ID
    )
    second_luna = next(
        model for model in second.json()["models"] if model["id"] == LUNA_ID
    )
    assert first_luna["availability"] == "both"
    assert second_luna["availability"] == "sponsored"
